...
if isempty(output)
    output=1:length(out_names);
else
    lenout = length(output);
    Vars = cell(1,lenout);
    for i=1:lenout
        Vars{i}=strcat('Out',num2str(output(i)));
    end
end
...
Creator = struct();
Creator.Ranges = Ranges;
Creator.ParIn = ParIn;

try
 ParT=addprop(ParT,{'Kind','rMethod','Fixed','output','Vars','Domain',...
     'Tout','MName','creator'},{'table','table','table','table','table','table',...
     'table','table','table'});
 ...
 ParT.Properties.CustomProperties.output=output;
 ParT.Properties.CustomProperties.Vars=out_names;
 ParT.Properties.CustomProperties.Domain=[min(tOut),max(tOut)];
 ParT.Properties.CustomProperties.Tout=tOut;
 ParT.Properties.CustomProperties.MName=model;
 ParT.Properties.CustomProperties.creator=Creator;
disp('All done!')
catch

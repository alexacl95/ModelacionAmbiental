function y = gsua_eval(par,Table,xdata,ydata,parallel)
...
if kind==1
    [y,~]=sens_montecarlo(Table,par,xdata,false,[0,size(par,1)]);%Possibly it is wrong
else
    fun=Table2.Solver;
    ni=Table2.NumVars;
    out=Table2.output;
    ...
    switch kind
        case 3
            ...
            if size(par,1)>2
                parfor (i=1:size(par,1),parforArg)
